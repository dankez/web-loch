    ...
    while (offset + 16 <= buffer.byteLength) {
      ...
      if (type === 3) {
          ...
          for (let i = 0; i < 5; i++) {
             ...
             console.log(`Leg ${i}: from=${fromId}, to=${toId}, flags=${flags}`);
          }
      }
      if (type === 4) {
          console.log(`\n${filename} TYPE 4 (Splays?):`);
          const chunkOffset = offset + 16;
          for (let i = 0; i < 5; i++) {
             const rOff = chunkOffset + i * 32;
             const fromId = view.getUint32(rOff, true);
             const typeFlags = view.getUint32(rOff + 4, true);
             const toId = view.getInt32(rOff + 8, true); // What if this is toId? wait, no.

             console.log(`Type 4 [${i}]: from=${fromId}, flags=${typeFlags}`);
          }
      }
